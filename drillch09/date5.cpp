...
// Függvény hónap hozzáadására vagy kivonására egy dátumhoz. 
{
    int new_m = n % 12;   // az évekből hónapok, növelni 
    int new_y = n / 12;   // éveket növelni 

    /* A hónap típusa gondoskodik magáról a túlcsordulás esetén,
     de elengedjük az év módosítását, ha ez megtörténik. 
     Ellenőriznünk kell, mielőtt ténylegesen működtetnénk az m-t */
    if (Month::dec < (int(m)+new_m)) ++new_y;
    if (Month::jan > (int(m)+new_m)) --new_y;
    m += new_m;   // m Month típusú
    y += new_y;
}

void Date::add_year(int n)
// Függvény évek hozzáadására vagy kivonására a dátumból. 
{
    y += n;
}
